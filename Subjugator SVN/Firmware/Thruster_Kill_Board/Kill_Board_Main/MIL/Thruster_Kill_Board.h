...
//CAN defines
//task group 5 ,ECU 2
#define TKB_CANID 0x12 //device ID
#define TKB_CAN_BASE CAN1_BASE

//task group 0, ECU 0
#define TKB_KILLID 0x10    //from any source
...
#define TKB_CAN_KILL_LEN 5 //1 byte for confirm kill, 1 byte for source code
...
#define SOFT_BYTE 0x53 //ASCII: 'S' SOFT
#define HARD_BYTE 0x48 //ASCII: 'H' HARD
/*Unassert/Assert(UA) Byte */
#define U_BYTE 0x55 //ASCII: 'U' UNASSERT
#define A_BYTE 0x41 //ASCII: 'A' ASSERT
/*Motherboard(M) Heartbeat Source Byte*/
#define MOBO_BYTE 0x4D //ASCII: 'M' MOTHERBOARD
/*Message Type Byte*/
#define KILL_START_BYTE 0x4B //ASCII: 'K'
#define HEARTBEAT_START_BYTE 0x48 //ASCII: 'H'
...
#define RV_KILL_PIN GPIO_PIN_0 //ON PORT F
#define LH_KILL_PIN GPIO_PIN_1 //ON PORT F
#define RH_KILL_PIN GPIO_PIN_2 //ON PORT F
#define LV_KILL_PIN GPIO_PIN_3 //ON PORT F

#define MAIN_PWR_PIN GPIO_PIN_4//ON PORT F

#define DBG_LED_PIN GPIO_PIN_0 //ON PORT D

//initialize kill pins
//default pins to 0 to kill power to thrusters
#define INIT_THRUST_PWR_IO() do{ \
        GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE, RV_KILL_PIN | LH_KILL_PIN | RH_KILL_PIN | LV_KILL_PIN);\
        GPIOPinWrite(GPIO_PORTF_BASE, RV_KILL_PIN | LH_KILL_PIN | RH_KILL_PIN | LV_KILL_PIN, 0x00);\
        }while(0)

//initialize main power (ON/OFF)
//default pin to 1 to power rest of sub
#define INIT_MAIN_PWR_IO() do{ \
        GPIOPinTypeGPIOOutput(GPIO_PORTF_BASE,MAIN_PWR_PIN);\
        GPIOPinWrite(GPIO_PORTF_BASE, MAIN_PWR_PIN, 0xFF);\
        }while(0);

#define KILL_THRUSTERS()  GPIOPinWrite(GPIO_PORTF_BASE, RV_KILL_PIN | LH_KILL_PIN | RH_KILL_PIN | LV_KILL_PIN, 0x00)
#define POWER_THRUSTERS() GPIOPinWrite(GPIO_PORTF_BASE, RV_KILL_PIN | LH_KILL_PIN | RH_KILL_PIN | LV_KILL_PIN, 0xFF)

#define KILL_MAIN() GPIOPinWrite(GPIO_PORTF_BASE, MAIN_PWR_PIN, 0x00)
#define POWER_MAIN() GPIOPinWrite(GPIO_PORTF_BASE, MAIN_PWR_PIN, 0xFF)
...
//ALL ON PORT B
#define HALL_HARDKILL_PIN   GPIO_PIN_0
